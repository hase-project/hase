...
class stpncpy(SimProcedure):
    def run(self, dst, src, n):
        ...
        ret_size = self.state.solver.If(self.state.solver.ULE(n, src_len_expr), n, src_len_expr)
...
class strncasecmp(SimProcedure):
    def run(self, a_addr, b_addr, limit):
        return self.state.solver.Unconstrained('strncasecmp', self.state.arch.bits, uninitialized=False)
...
class malloc_usable_size(SimProcedure):
    def run(self, ptr):
        ...
        return self.state.solver.Unconstrained('malloc_useable_size', self.state.arch.bits)
