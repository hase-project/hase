        ...
        try:
            ...
            if type(size) in (int, int):
                ...
                for k in range(size):
                    ...
                    if self.verbose: self.log("\tLoading from: " + str(hex(addr + k) if type(addr) in (int, int) else (addr + k)))
        ...
        try:
            ...
            if type(size) in (int, int):
                ...
                for k in range(size):
                    ...
                    if not flag and type(addr) not in (int, int):
    ...
    def _merge_symbolic_addresses(self, others, merge_conditions, verbose=False):
        ...
        for k in range(len(all)):
            ...
            for f, v, r in m._symbolic_memory:
                ...
                found = False
                for ff, V in list(formulas.items()):

                    # do we have the same _exact_ formula?
                    if ff is f:

                        for vv, mems in list(V.items()):
                            # same content?
                            if v.compare(vv):
                                mems.append(k)
                                found = True
        ...
        count_same_address = 0
        for f, V in list(formulas.items()):
