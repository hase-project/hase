    ...
    if args.filter == "auto":
        filters = ",".join(
            [
                ...
                if not all([item["valid"] for item in data[benchmark]["original"]])
                or not all([item["valid"] for item in data[benchmark]["hase"]])
            ]
        )
        if len(filters):
            args.filter = filters
...
if __name__ == "__main__":
    ...
    parser.add_argument("data", type=str, help="The json file that holds the result")
    parser.add_argument(
        "--time",
        type=str,
        default="overall",
    ...
    parser.add_argument("-f", "--filter", type=str, help="Abnormal suites")
    parser.add_argument(
        "-m", "--merge", type=str, help="Different results to merge together"
    )
    parser.add_argument(
        "-s", "--std", action="store_true", help="Report standard deviations"
    )
    parser.add_argument(
        "-i", "--intermediate", action="store_true", help="Report intermediate data"
    )
